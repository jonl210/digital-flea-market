...
from .models import Items, ItemSupplier, Customer, Item
import datetime
...
def addNewItem(request):
    if request.method == "POST":
        ...
        if request.POST.get("itemName") and request.POST.get("salePrice") and request.POST.get("itemMarkup") and request.POST.get(
                "priceToOrder") \
            ...
            supplier = ItemSupplier(ID=request.POST.get("supplierID"), SupplierName="Test Name Here2", ValueSuppliedToDate=0.0,
                                     FirstItemDate=datetime.datetime.now(), LastItemDate=datetime.datetime.now())
            supplier.save()
            ...
            newItem = Items(ItemSupplierID=supplier, ItemDiscription=request.POST.get("itemDescription"),
                             ...
                             ItemMarkup=request.POST.get("itemMarkup"), SalePrice=request.POST.get("salePrice"),
                             PriceToOrder=request.POST.get("priceToOrder"), \
...
def checkout_page(request):
    return render(request, "home/checkout
